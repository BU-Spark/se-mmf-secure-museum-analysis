...
(function (exports, node) {
  ...
  function jiff_compute_shares(jiff, secret, parties_list, threshold, Zp) {
    ...
    for (i = 0; i < parties_list.length; i++) {
      ...
      for (var j = 1; j < polynomial.length; j++) {
        ...
        power = jiff.helpers.mod(
          power.times(jiff.helpers.get_party_number(p_id)),
          Zp
        );
      }
    }
  ...
  function jiff_lagrange(jiff, shares) {
    ...
    for (i = 0; i < shares.length; i++) {
      ...
      for (var j = 0; j < shares.length; j++) {
        ...
        if (pj !== pi) {
          ...
          lagrange_coeff[pi] = jiff.helpers
            .mod(lagrange_coeff[pi].times(0 - pj), shares[i].Zp)
            .times(inv);
          lagrange_coeff[pi] = jiff.helpers.mod(
            lagrange_coeff[pi],
            shares[i].Zp
          );
        }
    ...
    for (i = 0; i < shares.length; i++) {
      pi = jiff.helpers.get_party_number(shares[i].sender_id);
      var tmp = jiff.helpers.mod(
        shares[i].value.times(lagrange_coeff[pi]),
        shares[i].Zp
      );
  ...
  function make_jiff(base_instance, options) {
    ...
    if (jiff.has_extension("negativenumber")) {
      throw new Error(
        "Please apply bignumber before negative number extensions"
      );
    }
    ...
    jiff.helpers.extended_gcd = function (a, b) {
      ...
      return (function recursive_helper(a, b) {
        if (b.isZero()) {
  ...
  typeof exports === "undefined" ? (this.jiff_bignumber = {}) : exports,
